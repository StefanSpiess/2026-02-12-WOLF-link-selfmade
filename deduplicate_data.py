...
def deduplicate_csv(csv_path="data/wolf.csv", dry_run=False):
    ...
    for row in rows:
        ...
        try:
            ...
            print(
                f"   ⚠️  Warning: Could not parse timestamp '{row.get('timestamp')}': {e}"
            )
            continue
    ...
    if not dry_run:
        ...
        print(
            f"   ✓ CSV deduplicated: {before_count} → {after_count} entries ({removed_count} removed)"
        )
    else:
        print(
            f"   [DRY RUN] Would remove {removed_count} duplicates: {before_count} → {after_count} entries"
        )
...
def deduplicate_database(db_path="data/wolf.db", dry_run=False):
    ...
    if not dry_run:
        ...
        print(
            f"   ✓ Database deduplicated: {before_count} → {after_count} entries ({deleted_count} removed)"
        )
    else:
        ...
        print(
            f"   [DRY RUN] Would remove {would_delete} duplicates: {before_count} → {after_count} entries"
        )
...
def show_statistics(csv_stats, db_stats):
    ...
    if csv_stats:
        ...
        if csv_stats["days_with_duplicates"]:
            ...
            for date, count in sorted(
                csv_stats["days_with_duplicates"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:10]:
    ...
    if db_stats:
        ...
        if db_stats["days_with_duplicates"]:
            ...
            for date, count in sorted(
                db_stats["days_with_duplicates"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:10]:
                print(f"     • {date}: {count} entries (removed {count-1})")
...
Note: This will OVERWRITE the original files. There is no backup!
    ...
    parser.add_argument("--csv-only", action="store_true", help="Only process CSV file")
    parser.add_argument(
        "--db-only", action="store_true", help="Only process SQLite database"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show statistics without modifying files"
    )
    parser.add_argument(
        "--csv-path",
        default="data/wolf.csv",
        help="Path to CSV file (default: data/wolf.csv)",
    )
    parser.add_argument(
        "--db-path",
        default="data/wolf.db",
